...
export default function CartListItem(props: any) {
    ...
    const { data: productDetails, isLoading: isLoadingProductDetails } = useProductDetails({
        productId: cartProduct && cartProduct.get('product') && cartProduct.get('product').get('productId')
    }, {
        refreshInterval: 0,
            ...
            <div className="flex align-items-center gap-2 p-3 flex-wrap lg:flex-nowrap">
                ...
                <div className="w-3rem lg:w-4rem">
                    {
                        productDetails ?
                            <img
                                src={`${systemConfig && productDetails &&
                ...
                <div className="w-10rem lg:w-15rem">
                    ...
                    <p
                        ...
                        style={{ color: "#9D9D9D" }}
                    >
                        {getLanguageBaseName(productDetails && productDetails.get('description'))}
                    </p>
                    <p className="flex align-items-center gap-2 m-0">
                        ...
                        <span className="text-xs" style={{ color: "#9D9D9D" }}>
                            <del> {productDetails && productDetails.get('basePrice')}</del>
                        </span>
                    </p>
                </div>
